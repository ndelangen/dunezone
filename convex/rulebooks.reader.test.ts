...
describe('Rulebook current-Edition reader', () => {
  ...
  test('reads the current Edition number and never falls back to an older Edition or draft', async () => {
  ...
  test.each(['rulebook', 'ruleset'] as const)(
    ...
    async (kind) => {
      ...
      expect(await t.query(api.rulebooks.readerPage, { ...locator, rulebook_slug: 'missing' })).toBeNull();
      expect(
        await t.query(api.rulebooks.readerPage, { ...locator, ruleset_slug: 'other-rulebook-test-rules' })
      ).toBeNull();
      await t.run(async (ctx) => {
        if (kind === 'rulebook') {
          await ctx.db.patch('rulebooks', created.rulebook._id, { is_deleted: true });
        } else {
  ...
  test('resolves published images referenced by the Edition and omits deleted Assets', async () => {
    ...
    const assetId = await t.run(async (ctx) => {
      ...
      await ctx.db.patch('rulebook_editions', created.edition._id, { contents });
      return id;
    });
    expect(await t.query(api.rulebooks.readerPage, locator)).toMatchObject({
