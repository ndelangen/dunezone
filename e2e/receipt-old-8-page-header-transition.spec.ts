...
test('old, rate 8, run 1: the persistent page hero contracts when navigating to a headerless route', async ({ page }) => {
...
test('old, rate 8, run 2: the persistent page hero contracts when navigating to a headerless route', async ({ page }) => {
  await page.goto('/privacy');
  const hero = page.getByRole('banner');
  await expect(hero).toBeVisible();
...
test('old, rate 8, run 3: the persistent page hero contracts when navigating to a headerless route', async ({ page }) => {
  await page.goto('/privacy');
  const hero = page.getByRole('banner');
  await expect(hero).toBeVisible();
