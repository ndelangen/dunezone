...
later the panel spoke three vocabularies (a raw `.button`, a Mantine `Button`, the imported section
and a kit `Card` on a pane of its own), and the kit `Section` a fourth had tried rendered its title
at 1.3:1 because the shell pinned none of the tokens it reads. So the line is drawn at what the kit
...
provider re-emits Mantine's root-scoped scheme variables under it). The panel paints its own ground,
so its content sits inside `PaintedSurfaceBoundary` and nothing on it brings a pane of its own.
...
*Enforced by `check:css-orphans`, which reads route stylesheets, and by the `PanelInLightScheme`
story in [`hosted.route.stories.tsx`](../../src/app/routes/_app/play/hosted.route.stories.tsx); the
parts ledger and the built-with line are convention, recorded on the play map's Notes.*
