...
| Owner | Location | Rule |
|---|---|---|
| Components | `src/app/ui/**` (alias `@ui/*`) | Every published component, filed by category. One rule: it renders what it is given, so no Convex client, no `@db` values (types are fine), no router data hooks. Composes Mantine under `appContentTheme`. |
