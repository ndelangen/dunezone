...
Zod) would drive type safety on both sides, Convex and the app, and that
a happy-flow e2e test would provide the real confidence signal. Feature work
...
Sequencing rule: when retiring mid-stack tests, reinforce the top first.
An e2e spec or type guarantee must exist before the tests it supersedes
are deleted, and deletions are listed with what now carries their
confidence.

**Exemption:** the publisher/ops regression suites (font, capture
contract, deployment contract, live drift) stay as-is, because they
guard rendered output and infrastructure state that no type system or
e2e spec can express.
