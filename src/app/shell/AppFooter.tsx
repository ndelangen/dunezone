...
/**
 * One preference as a labelled three-segment radio group. Both footer preferences (motion, color scheme) share this
 * single UI: System defers to the OS hint, the other two segments pin it.
 */
...
/**
 * Public waypoints to the project's component catalogue, source, policies, and community homes — and the controls that
 * override the OS's appearance hints for this site: reduced motion (see `motion.ts`) and the color scheme (see
 * `colorScheme.ts`). The waypoints are icon-only circles; one `label` per entry fans out to the accessible name and the
 * `Tooltip`, so the two cannot come apart. The preference controls are bare inputs styled with the band rather than the
 * content theme; both share the segmented radio treatment.
 */
