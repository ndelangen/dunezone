 ...
 *
 * This hook owns the mechanics and nothing visual: the countdown (a ref as source of truth, mirrored into state for display, firing from inside a state updater would make it impure, and React batches and may re-invoke updaters, either of which turns one completed hold into a wrong number of firings);
 * the latch that keeps a finished hold from firing twice before the caller's `pending` arrives, its render-phase reset when the round trip ends so a failed action gets its retry back, and the handler bag: only a primary press holds, touch releases its implicit capture so drag-off cancels, blur cancels, a repeated keydown does not restart, only the activation key's release cancels, and the context menu stays closed mid-hold.
 *
