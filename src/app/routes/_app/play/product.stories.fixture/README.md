...
- Troop discs and Traitor fronts: rendered through the existing game renderers using the copied
  ...
  publish those individual faces. `preparedSnapshot` deals Traitors from the first nine leaders in
  faction order, so only those nine fronts are kept: `house-atreides-traitor-0` to `-4` and
  `house-harkonnen-traitor-0` to `-3`. The other 21 fronts were local renderer captures too. They
  return through the published components of
...
- Treachery cards: the card back and the Snooper face, copied from the published Dreamrules deck
  into `.storybook/static/play-fixtures/dreamrules`.- Players: the public profile snapshot recorded in `drafting.stories.fixture.ts`. Their public
