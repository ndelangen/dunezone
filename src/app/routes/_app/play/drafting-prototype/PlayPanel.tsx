...
function Log({ state, scenario }: { state: PlayState; scenario?: LogScenario }) {
  const records =
    scenario === 'log-empty'
      ? []
      : scenario === 'log-older'
        ? LOG_EXAMPLES.slice(6)
        : scenario
          ? LOG_EXAMPLES
          : state.log;
  const group = state.left[1] === 'audit' ? 'audit' : 'game';
  const entries = records.filter(
    (entry) => (entry.kind === 'seat' || entry.kind === 'vote' ? 'audit' : 'game') === group
  );
  return (
    <div className="dpl-log" data-log-variant="log" data-log-group={group}>
      <ol className="dpl-log__list" aria-label="Game log">
        {entries.map((entry, index) => (
          <li key={index}>
            <Badge variant="light" color={LOG_CLASSIFICATIONS[entry.kind].color} size="sm" tt="none">
              {LOG_CLASSIFICATIONS[entry.kind].label}
            </Badge>
            <div>
    ...
    <div className="dpl-thread">
      ...
      >
        <TextInput
          ...
          miw={0}
          value={state.draft}
          placeholder={`Message ${factionName(faction)}`}
          aria-label="Message"
          onChange={(event) => dispatch({ type: 'draftMessage', text: event.target.value })}
        />
        <Button size="sm" type="submit" disabled={!state.draft.trim()}>
          Send
        </Button>
      </form>
    </div>
    ...
    <div className="dpl-spice">
      ...
      <div className="dpl-battle__row">
        ...
        <button
          ...
          onClick={() => dispatch({ type: 'setSpawnAmount', amount: state.spawnAmount - 1 })}
        >
          −
        </button>
        ...
        <button
          ...
          disabled={state.spawnAmount > state.bank}
          onClick={() => dispatch({ type: 'spawnSpice' })}
        >
          Spawn on the table
        </button>
      </div>
      <p className="dp-hint">
        The stack lands in front of you. Move it to whoever it is for; they pick it up into their bank. Every stack on
        the table is public.
      </p>
        ...
        {state.stacks.map((stack) => (
          <li key={stack.id}>
            ...
            <span className="ds-items__name">
              {stack.amount} spice{' '}
              <small>
                {stack.from === me?.faction ? 'spawned by you' : `from ${factionName(stack.from)}, in front of you`}
              </small>
            </span>
            <button
              type="button"
              className="dp-swap-action is-accept"
              onClick={() => dispatch({ type: 'pickUpSpice', stack: stack.id })}
            >
              Pick up
            </button>
          </li>
...
function PublicPane({ state, faction }: { state: PlayState; faction: string }) {
  ...
  const stacksFromThem = state.stacks
    .filter((stack) => stack.from === faction)
        ...
        <dd>
          {faction === 'bene-gesserit'
            ? state.prediction.revealed
              ? `revealed: ${factionName(state.prediction.faction ?? '')}, turn ${state.prediction.turn}`
...
function useSplit() {
  ...
  const onPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const move = (moveEvent: PointerEvent) =>
      setSplit(Math.max(28, Math.min(72, ((moveEvent.clientX - bounds.left) / bounds.width) * 100)));
    const up = () => {
...
export function PlayPanel({
  ...
  const leftItem = (key: string, label: string, icon: ReactNode) => (
    <NestedTabs.Item
      ...
      label={label}
      icon={icon}
      onClick={() => dispatch({ type: 'setLeft', path: [key] })}
    />
  );
  const rightItem = (key: string, label: string, icon: ReactNode) => (
    <NestedTabs.Item
      as="button"
      type="button"
      path={[rightFaction, key]}
      label={label}
      icon={icon}
      onClick={() => dispatch({ type: 'setRight', path: [rightFaction, key] })}
    />
  );
  return (
      ...
      <NestedTabs activePath={leftPath} ariaLabel="Yours" className="dpl-tabs">
        ...
        {left === 'log' ? (
          <NestedTabs.Level label="Log">
            <NestedTabs.Item
              ...
              path={['log', 'game']}
              label="Game"
              icon={<TopicIcon topic="game" size={22} />}
              onClick={() => dispatch({ type: 'setLeft', path: ['log', 'game'] })}
            />
            <NestedTabs.Item
              as="button"
              type="button"
              path={['log', 'audit']}
              label="Audit"
              icon={<TopicIcon topic="audit" size={22} />}
              onClick={() => dispatch({ type: 'setLeft', path: ['log', 'audit'] })}
            />
          </NestedTabs.Level>
        ) : null}
        <NestedTabs.ContentPanel aria-label="Yours">
          <div className={`dpl-content${left === 'log' ? ' dpl-content--log' : ''}`}>
            {left === 'hand' ? (
              <>
                <Hand state={state} dispatch={dispatch} size={0.2} />
                {battleHand}
              </>
            ) : null}
