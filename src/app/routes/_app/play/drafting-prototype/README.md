...
### `?variant=play&battle=battle`: the accepted battle

Norbert [accepted K](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653592014) on 2026-09-13. It is the sole live battle arrangement, now named `battle`. `?variant=play` also selects it by default. The floating controls retain the scenario and viewer selectors; comparison arrows and letter variants are removed. Run `bun run app:dev` and open `/play/demo?variant=play&battle=battle&scenario=fan`.

Each part embodies a recorded choice:

- `BattleCallout` composes the shallow capsule, small centred lower tab and broad pointer from [the corrected sketch](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653471249). The public UI stays minimal, without helper text or a territory name. `BattleScene3D` uses the dropped marker's coordinates, as [requested with the north-facing reveal](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652841462), and keeps the callout visible in the four table views.
- `BattleWheel` and `BattleCardFan` are shared route organs for the private preview and public reveal. They embody [the editor visualization](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5614676247) and [its use in the public reveal](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652912424): north-facing circles, vertical cards fanned behind, actual troop discs and counts to the left, spice above the complete leader to the right, and calculated troop strength at the top.
- `ReadinessRing` embodies [the closer plans and perimeter readiness](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652980673). The capsule is 500px wide with a 72px centre column. Preparing pulses amber; ready is steady green. Accessible text names the faction and state. The centre holds the countdown. Rings leave when the plans reveal. These are explicitly requested state indicators.
- The reveal retains [the accepted animation](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652912424): a simultaneous 720ms flip, then cards and actions fading in. Private faces mount only at reveal. Reduced motion removes the motion.
- The outcome controls embody [the fitted button corners](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653554550). No winner occupies the countdown's centre. The two win buttons sit inside the lower tab, each following its outer lower corner with an 18px radius, a 6px inset from the tab's 24px curve and a 4px gap between buttons. Cancel follows both lower corners before countdown. The tab remains through countdown to keep the silhouette steady.
- `BattlePlanner` retains the controls beside their shared wheel preview in the accepted play panel. Ready and Undo Ready stay private. The [battle contract](https://github.com/ndelangen/dunezone/issues/1062#issuecomment-5620858736) governs claims, private plans, the five-second countdown, public provisional choices, agreement and manual cleanup. Aggressor is left. Matching choices resolve the fixture; it does not calculate casualties.

`battle.ts` owns temporary in-memory state. The scenario selector offers `marker`, `claim`, `unclaimed`, `pending`, `fan`, `countdown`, `revealed`, `revealed-south`, `outcome` and `settled`. The viewer selector acts as either combatant or inspects the spectator's public view. Cards and leaders drag from the reveal into the native 3D table. The existing table controls then move, flip and stack the pieces. Owned cards can also return by double-click before agreement. Agreement keeps placed pieces where they are, returns remaining cards to their respective hands and places remaining leaders near the indicator. The overlay disappears. The Hand tab opens to show returned cards. Spectators cannot move these pieces or inspect the private hand.

This is a fixed Fremen versus Atreides layout fixture, not a game authority, persistence layer or full inventory implementation. It uses one actual troop type per side, strength one funded or one half unfunded, cost one, and two selected cards in the `fan` scenario. These are sample inputs, not gameplay limits or authoring defaults. Leader movement and discard gestures remain delivery work. The private controls do not settle further faction-authoring questions.

The branch has no original battle-wheel artwork or dedicated battle-wheel renderer. The permitted fallback composes real `BackgroundRenderer`, `TroopToken`, complete `LeaderToken` and `TreacheryCard` with Advokat Modern numbers and Copperplate Gothic labels. Original wheel artwork remains a delivery concern; this composition does not replace that accepted design.

Reused vocabulary includes Mantine buttons and form controls, kit `StatusBadge`, `TopicIcon`, `NestedTabs`, and the real faction token and game renderers. `CalloutSurface` remains a story-covered kit Surface candidate on this unmerged branch. It owns the capsule, attached action tab and pointer; its caller supplies slots and pointer geometry. Battle wheel Content and claim Control are possible delivery extractions, not additional published kit components in this prototype. The branch's existing `NestedTabs` one-level support, `TopicIcon` additions and `NoticeMark` also remain candidates. This prototype restores the two-level tab root's layout properties misplaced by an earlier brace. K's fitted Mantine buttons add no kit component.

The [2026-09-14 corrections](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5666483733) reuse `WorkbenchLayout` and `CanvasScale` for the planner, with the same Mantine controls and typography as the kit. `NestedTabs` owns the sole content inset. The redundant faction and Private plan heading is removed. Conversation uses `TextInput` and `Button`; Ready uses the same button treatment. `TopicIcon.publicState` uses the existing Info glyph, a kit candidate on this branch. Play no longer copies setup-ready dots onto the faction tokens.

`battleTable.ts` maps fixture pieces to the existing local `TabletopProvider`. Optional face URLs give the native card and marker meshes their real artwork without changing the shared game schema. The card PNGs in `battle-cards` are isolated captures of the real `TreacheryCard` renderers. Leaders use their published renderer images. This adapter is local prototype code, not multiplayer delivery. It preserves placed pieces across the next battle and uses a new generation for their identities. The native placement path handles card-bay snapping, collisions and stacks. A table-physics candidate recognizes the existing tanks shelf as support; the old circular limit pulled leaders back off that shelf. The accepted north-facing reveal and fitted action tab remain.

`shots/battle.gif` is the accepted K comparison. `shots/battle-corrections.png` shows the corrected controls and resolved 3D table. The branch is never merged. Rejected variants and their images remain in the tags below.
