...
The floating switcher changes `?battle=` while keeping `?variant=play`; arrow keys cycle A through I outside editable controls. The scenario selector offers `marker`, `claim`, `unclaimed`, `pending`, `fan`, `countdown`, `revealed`, `revealed-south`, `outcome` and `settled`. The viewer selector is a fixture tool for acting as either combatant or inspecting the spectator's battle view.
...
- I, Integrated actions: follows Norbert's [request to remove the separate button background](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653381547). The two win actions sit beneath their plans inside the capsule. No winner occupies the countdown's centre position. Cancel uses that same centre while preparing. The separate action tab is absent in every I state. Space for the win actions is reserved so the shape stays steady through the reveal. I preserves H's 500-pixel width and readiness rings, the shared editor visualization and the accepted animation. The comparison remains open.
