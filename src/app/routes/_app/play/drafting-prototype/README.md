...
The floating switcher changes `?battle=` while keeping `?variant=play`; arrow keys cycle A through J outside editable controls. The scenario selector offers `marker`, `claim`, `unclaimed`, `pending`, `fan`, `countdown`, `revealed`, `revealed-south`, `outcome` and `settled`. The viewer selector is a fixture tool for acting as either combatant or inspecting the spectator's battle view.
