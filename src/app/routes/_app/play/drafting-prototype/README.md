...
`battle.ts` owns temporary in-memory state. The scenario selector offers `marker`, `claim`, `unclaimed`, `pending`, `fan`, `countdown`, `revealed`, `revealed-south`, `outcome` and `settled`. The viewer selector acts as either combatant or inspects the spectator's public view. Cards can be dragged from the reveal to a table position and owned cards returned by double-click. Agreement retains moved cards in their positions and settles the remaining cards and leaders near the marker.
...
`shots/battle.gif` is the accepted K comparison. The branch is never merged. Rejected variants and their images remain in the tags below.
