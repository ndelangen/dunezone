...
The floating switcher changes `?battle=` while keeping `?variant=play`; arrow keys cycle A through K outside editable controls. The scenario selector offers `marker`, `claim`, `unclaimed`, `pending`, `fan`, `countdown`, `revealed`, `revealed-south`, `outcome` and `settled`. The viewer selector is a fixture tool for acting as either combatant or inspecting the spectator's battle view.
...
- K, Fitted buttons: follows Norbert's [button-shape refinement](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653554550). The win buttons have a 6px inset from the tab and an 18px outer lower radius within its 24px curve. The left and right buttons follow their respective outer corners, with small inner corners around a 4px gap. Cancel follows both lower corners. K retains J's silhouette and the accepted centre action, rings and reveal animation. This changes route composition of Mantine buttons, with no new kit candidate. Final acceptance is open.
- J, Sketch silhouette: follows Norbert's [shape correction](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653471249). A shallow capsule and a small centred lower tab share the CalloutSurface paint. The two win buttons sit in that tab; No winner uses the countdown's centre between the plans. The tab remains through preparing and countdown, with Cancel before countdown. The accepted rings, spacing, north-facing editor visualization and reveal animation remain. Final acceptance is open. This reuses the existing CalloutSurface candidate without changing the kit.
- I, Integrated actions: rejected by Norbert in the next sketch correction. This variant follows Norbert's [request to remove the separate button background](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5653381547). The two win actions sit beneath their plans inside the capsule. No winner occupies the countdown's centre position. Cancel uses that same centre while preparing. The separate action tab is absent in every I state. Space for the win actions is reserved so the shape stays steady through the reveal. I preserves H's 500-pixel width and readiness rings, the shared editor visualization and the accepted animation. The comparison remains open.
- H, Readiness rings: follows Norbert's [spacing and readiness refinement](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652980673). The capsule narrows from 588 to 500 pixels, leaving a 72-pixel centre column for the countdown. Each claimed side has a readiness ring: amber pulsing over 1.6 seconds while preparing, steady green when ready. Readiness text remains available to assistive technology; the visible centre is reserved for the countdown. Rings leave at reveal. Reduced motion makes the preparing ring steady. The functional ring is explicitly requested, not an added decorative border. H keeps the accepted reveal animation and shared editor visualization. The comparison remains open.
- G, Editor visualization: tries the [accepted editor wheel](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5614676247) in the public reveal. `BattleWheel` and `BattleCardFan` are shared route organs for the editor and reveal. Selected cards fan behind each circle; the left readout shows the actual troop disc and count without the previous dialed and undialed captions. Spice sits above the complete leader token on the right. The `fan` scenario gives both plans two selected cards so the composition is visible; this is not a card-count rule. The fixture still has one troop type, so its wheel has one troop row, rather than inventing troop types to match the sketch's example rows. Public cards remain movable and returnable after reveal. Norbert [accepted the animation](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652912424); G retains the 720-millisecond disc flip and accompanying fades. The visualization comparison remains open.
- F, North-facing reveal: follows Norbert's [revised direction](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652841462). Both wheels face north and both card columns stay vertical. The capsule narrows from 650 to 588 pixels while retaining its circle inset. It displays no territory name. Centred faction text such as "Fremen preparing" and "Atreides ready" gives way to the countdown in the same position. At reveal, both discs flip together over 720 milliseconds, with cards and actions fading into place. The private leader and cards mount only at reveal. Reduced motion makes the change immediate. The marker can be dragged to any point inside the board; its ray-projected coordinates become the anchor without a territory lookup. F remains a proposal.
- E, Facing plans: applies Norbert's [minimal-overlay corrections](https://github.com/ndelangen/dunezone/issues/1016#issuecomment-5652412398). The left wheel and cards turn 90 degrees clockwise; the right wheel and cards turn 90 degrees counterclockwise. The capsule has a 22-pixel inset around the 180-pixel circles and a 112-pixel radius. It keeps the territory name, readiness or chosen result, and filled action buttons. Explainer and helper text is removed, including the battle planner's empty-state guidance. The composite shows revealed, unrevealed and fully unclaimed states. E remains a proposal and keeps A's planner arrangement pending that separate choice.
- D, Territory capsule: follows Norbert's two original sketches supplied on 2026-09-13. One capsule holds both combatant circles and a card column for each side. The three result controls sit on an attached tab. A broad pointer connects to the territory; the capsule sits on the opposite side in the available view. The southern anchor is a placement fixture, not a claim about a named territory's coordinates. Card stacks support the selected cards, without a three-card limit. Sketch colours are not treated as a new palette decision. D temporarily reuses A's private planner. Neither D nor a private planner has been accepted.
...
Reused kit: `NestedTabs`, `Surface`, `TopicIcon`; controls use Mantine directly. The proposed vocabulary is a read-only battle-wheel Content, a claim Control and a data-only battle-summary Block. Their route compositions remain organs here, pending acceptance and delivery. `CalloutSurface` is a new Surface candidate with a story, kept on this unmerged branch. It owns the capsule, attached action tab and broad pointer; callers supply content and pointer geometry. It adds no outline. Its radius can follow the content inset, and it omits the action tab while the countdown leaves no public action. The branch's existing `NestedTabs` one-level support and `TopicIcon` additions remain candidates; this work restores the two-level root's layout properties misplaced by the earlier brace.
