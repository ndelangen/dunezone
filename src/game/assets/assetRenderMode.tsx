...
/**
 * Ambient render mode (#254): `display` everywhere by default; the print preview route and the
 * publisher capture wrap their trees in `print` so the same components resolve print-grade variants
...
export const AssetRenderModeProvider: FC<{ mode: AssetRenderMode; children: ReactNode }> = ({
  mode,
  children,
}) => <AssetRenderModeContext.Provider value={mode}>{children}</AssetRenderModeContext.Provider>;

/**
 * Resolve an asset key in the ambient render mode. An explicit `size` wins over the mode
 * (thumbnails are `small` in every world).
 */
