...
function targetConvex(deployment: TargetDeployment, args: string[], env: NodeJS.ProcessEnv) {
  const convexEnv =
    deployment.kind === 'self-hosted'
      ? selfHostedEnvironment(env, deployment)
      : cloudDevEnvironment(env, deployment);
  return run('bunx', ['convex', ...args], { env: convexEnv });
}
...
export async function backendUp(
  ...
  if (options.adminKeyPersistPath) {
    mkdirSync(dirname(options.adminKeyPersistPath), { recursive: true });
...
const prepareLocalUsers = makeFunctionReference<
  'mutation',
  { ownerEmail: string; collaboratorEmail: string },
  { ownerId: string; collaboratorId: string }
>('provisioning:prepareLocalUsers');
const remapFactionBatch = makeFunctionReference<
  'mutation',
  { ownerEmail: string; cursor: string | null },
  RemapBatchResult
>('provisioning:remapFactionOwnershipBatch');
const remapGroupBatch = makeFunctionReference<
  'mutation',
  { ownerEmail: string; collaboratorEmail: string; cursor: string | null },
  RemapBatchResult
>('provisioning:remapGroupOwnershipBatch');
 ...
 * data (ticket #357).
 */
export async function remapOwnershipToLocalUsers(
  deploymentUrl: string,
  ownerEmail: string,
  collaboratorEmail: string
) {
  const client = new ConvexHttpClient(deploymentUrl);
  await client.mutation(prepareLocalUsers, { ownerEmail, collaboratorEmail });

  let cursor: string | null = null;
  while (true) {
    const result: RemapBatchResult = await client.mutation(remapFactionBatch, {
      ownerEmail,
      cursor,
    });
    if (result.isDone) {
  ...
  cursor = null;
  while (true) {
    const result: RemapBatchResult = await client.mutation(remapGroupBatch, {
      ownerEmail,
      collaboratorEmail,
      cursor,
    });
...
export function parseProvisionArgs(argv: string[]): ProvisionArgs {
  ...
  if (target !== 'e2e' && target !== 'local' && target !== 'dev') {
  ...
  return { target, stages: stages.length > 0 ? stages : allowed };
}
...
async function runCli(args: ProvisionArgs) {
  ...
  let deployment: SelfHostedDeployment | null = null;
  ...
  if (args.stages.includes('backend')) {
    console.log('Resetting the disposable local Convex backend...');
    deployment = await backendUp(env, backendOptions);
  } else {
    ...
    deployment = { kind: 'self-hosted', url, adminKey };
  }

  if (args.stages.includes('configure')) {
...
if (import.meta.main) {
  runCli(parseProvisionArgs(process.argv.slice(2))).catch((error: unknown) => {
