...
describe('assert-e2e-shards-cover', { timeout: TEST_BUDGET_MS }, () => {
  test('passes when every spec is listed exactly once', async () => {
    const verdict = await gate(['a.spec.ts', 'b.spec.ts', 'c.spec.ts'], {
      first: ['e2e/a.spec.ts'],
  ...
  test('fails on a spec no shard lists, naming it', async () => {
    const verdict = await gate(['a.spec.ts', 'b.spec.ts'], { first: ['e2e/a.spec.ts'] });
    expect(verdict.code).toBe(1);
    expect(verdict.stderr).toContain('e2e/b.spec.ts is assigned to no shard');
  });

  test('fails on a spec two shards list', async () => {
    const verdict = await gate(['a.spec.ts'], { first: ['e2e/a.spec.ts'], second: ['e2e/a.spec.ts'] });
    expect(verdict.code).toBe(1);
    expect(verdict.stderr).toContain('e2e/a.spec.ts is assigned to shards first and second');
  });

  test('fails on a listed file that does not exist', async () => {
    const verdict = await gate(['a.spec.ts'], { first: ['e2e/a.spec.ts', 'e2e/gone.spec.ts'] });
    expect(verdict.code).toBe(1);
  ...
  test('fails on a list entry that is not a spec file directly under e2e', async () => {
    const verdict = await gate(['a.spec.ts'], { first: ['e2e/a.spec.ts', '../package.json'] });
    expect(verdict.code).toBe(1);
  ...
  test('fails on a shard name that would not read in the Actions UI', async () => {
    const verdict = await gate(['a.spec.ts'], { 'Shard 1': ['e2e/a.spec.ts'] });
    expect(verdict.code).toBe(1);
