  ...
  return {
    ...
    fetcher: async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.pathname.endsWith('/settings')) {
        result = {
          bindings: [
            { name: 'GAME_ROOMS', type: 'durable_object_namespace', namespace_id: namespaceId },
      ...
      } else if (url.pathname.endsWith('/secrets') || url.pathname.endsWith('/domains')) {
        result = [];
      } else if (url.pathname.endsWith('/routes')) {
        result = options.route ? [{ pattern: 'other.example/*' }] : [];
      } else if (url.pathname.endsWith('/schedules')) {
        result = { schedules: [] };
      } else if (url.pathname.endsWith('/subdomain')) {
        result = { enabled: options.public ?? false, previews_enabled: false };
      } else if (url.pathname.endsWith('/namespaces')) {
        result =
          Number(url.searchParams.get('page')) === (options.namespacePages ?? 1)
            ? [
                {
      ...
      return Response.json({ success: true, result, result_info: { total_pages: options.namespacePages ?? 1 } });
    },
  };
}
