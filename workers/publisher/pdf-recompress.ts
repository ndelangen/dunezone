 ...
 *
 * Chromium embeds every image as a lossless FlateDecode raster (see the spike record on the map, #256). The accepted
 * policy shrinks only the big RGB portrait rasters by LOSSLESS downsampling — resize to 0.35×, re-deflate, still
 * FlateDecode. JPEG is banned: at any quality it replaces this art style's grain with directional streaks. Grayscale
 * rasters (texture tiles), mask pairs, and small fragments are byte-untouched — tile grain is faction identity.
 *
...
/**
 * Area-averaging downsample of packed RGB rows — the correct filter for large reductions (every source pixel
 * contributes to exactly the output pixel(s) its area overlaps, weighted by coverage).
 */
